...
def pa_widget(markup_dir="Downloads/parking", output_dir=""):
    class Annotator(object):
        ...
        def mouse_click(self, event):
            if not event.inaxes:
                return
            if annotation_flag.value == "1":
                x, y = event.xdata, event.ydata

                self.xdata.append(x)
                self.ydata.append(y)
                line = Line2D(
                    [self.xdata[-2], self.xdata[-1]], [self.ydata[-2], self.ydata[-1]]
                )
                line.set_color("r")
                self.axes.add_line(line)

                if len(self.xdata) % 4 == 0:
                    self.annotation_dict[self.image].append(
                        [self.xdata.copy(), self.ydata.copy()]
                    )
    ...
    def paint_button_clicked(b):
        if annotation_flag.value == "0":
            annotation_flag.value = "1"
            button_paint.style = {"button_color": "red", "color": "white"}
        else:
            annotation_flag.value = "0"
    ...
    images = [
        f
        for f in listdir(markup_dir)
        if isfile(join(markup_dir, f)) and f[-4:] == ".jpg"
    ]
    ...
    annotation_flag = widgets.Label(value="0")
    ...
    button_paint = widgets.Button(
        description="🖌", style={"button_color": "#eeeeee"}, layout={"width": "35px"}
    )
